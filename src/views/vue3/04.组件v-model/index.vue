	...
	<el-row class="h-full">
		<el-col :span="20">
			<doc title="组件 v-model">
				...
				<p>要让这个例子实际工作起来，<Tag data="<CustomInput>" /> 组件内部需要做两件事：</p>

				<div class="tip">
					<p>1.将内部原生 <Tag data="<input />" />元素的 value attribute 绑定到 modelValue prop</p>
			...
			<doc title="computed 拦截prop">
				<p class="tip">通过 computed 实现，通过 get 获取父组件传过来的值，利用 set 更新触发emit事件</p>
				...
				<el-row class="flex items-center mb-2">
					...
					<el-col>
						<h2>父组件的 msg 值：<tag :data="msg" /></h2>
					</el-col>
				</el-row>
			</doc>
			<doc title="Proxy 代理对象">
				<Tip title>
					<p>使用 computed 拦截对象时，不会触发<code> computed </code>的<code>set</code>方法</p>
					<p>原因是：form.xxx = xxx时，并不会触发 computed 的set，只有form = xxx时，才会触发set</p>
				</Tip>
			...
			<doc title="其他">
				<Tip title>
					...
					<p>2.Vue 3.3 新特性 <Tag data="defineModel" />,但是目前是试验性的，需要 config 中配置一下</p>
				</Tip>
			</doc>
...
import { code1, code2, code3, code4, code5, computedCode, code6, defineModelCode, defineModelCode2 } from './code';
